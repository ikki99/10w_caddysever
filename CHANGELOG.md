...
- **修复** - 刷新页面跳转到登录页的问题
- **修复** - 新建文件夹位置错误的问题
- **新增** - `/api/auth/check` 专用 Session 检查接口
- **改进** - `initializeFilePath()` 错误处理
- **改进** - 更严格的 Cookie 安全设置

#### 详细说明
**问题：** 之前使用 `/api/system/info` 检查 Session，但该接口不需要认证，导致检查不准确。

**解决方案：**
1. 新增 `CheckAuthHandler` 专用认证检查
2. 修改 `checkFirstRun()` 使用新接口
3. 改进文件路径初始化容错机制
...
[1.0.0]: https://github.com/ikki99/10w_caddysever/releases/tag/v1.0.0
